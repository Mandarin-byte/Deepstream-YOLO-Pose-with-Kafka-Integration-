...
/**
 * Function to draw pose keypoints and skeleton on the frame
 */
static void
                     ...
                     NvDsObjectMeta *obj_meta, guint pipeline_width, guint pipeline_height)
{
  ...
  float *mask_data = (float *) obj_meta->mask_params.data;
  ...
  for (guint i = 0; i < num_joints; i++) {
    float xc_raw = mask_data[i * 3 + 0];
    float yc_raw = mask_data[i * 3 + 1];
    float confidence = mask_data[i * 3 + 2];
    ...
    if (display_meta == NULL || display_meta->num_circles == MAX_DISPLAY_META_ELEMENTS) {
  ...
  /* Draw skeleton lines */
  guint num_skeleton_lines = sizeof(skeleton) / sizeof(skeleton[0]);
  for (guint i = 0; i < num_skeleton_lines; i++) {
    int idx1 = skeleton[i][0] - 1;
    int idx2 = skeleton[i][1] - 1;

    if (idx1 >= (int)num_joints || idx2 >= (int)num_joints) continue;

    float x1_raw = mask_data[idx1 * 3 + 0];
    float y1_raw = mask_data[idx1 * 3 + 1];
    float conf1 = mask_data[idx1 * 3 + 2];
    float x2_raw = mask_data[idx2 * 3 + 0];
    float y2_raw = mask_data[idx2 * 3 + 1];
    float conf2 = mask_data[idx2 * 3 + 2];
    ...
    if (display_meta == NULL || display_meta->num_lines == MAX_DISPLAY_META_ELEMENTS) {
      display_meta = nvds_acquire_display_meta_from_pool(batch_meta);
      nvds_add_display_meta_to_frame(frame_meta, display_meta);
    }
